...
class Test_Workflow_Manager extends WP_UnitTestCase {
	...
	public function test_status_manager_inicia_componentes() {
		...
		$mockNotification = Mockery::mock( NotificationManager::class );
		$mockRedirection  = Mockery::mock( PostRedirectionManager::class ); 

		// 2. Preparar ambiente
		set_current_screen( 'dashboard' ); 

		// 3. Executar o método da classe testada (PASSANDO O NOVO ARGUMENTO)
		...
		$this->assertEquals( 
			...
			'O blocker deve ser hookado no init'
		);

		$this->assertEquals( 
			10, 
			has_action( 'init', array( $mockRoleMgr, 'register' ) ),
			'O role manager deve ser hookado no init'
		);
		
		$this->assertEquals( 
			10, 
			has_action( 'init', array( $mockNotification, 'register' ) ),
			'O notification manager deve ser hookado no init'
		);

		// NOVA ASSERÇÃO
		$this->assertEquals( 
			10, 
			has_action( 'init', array( $mockRedirection, 'register' ) ),
			'O redirection manager deve ser hookado no init'
		);
