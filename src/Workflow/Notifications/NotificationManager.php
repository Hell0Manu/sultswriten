...
class NotificationManager {
    ...
    public function notify_author_on_status_change( string $new_status, string $old_status, \WP_Post $post ): void {
        ...
        if ( $new_status === StatusConfig::REQUIRES_ADJUSTMENT ) { 
            ...
            $this->mailer->send(
                ...
                sprintf( '<p>Olá! O artigo <strong>"%s"</strong> foi revisado e retornou para ajustes. Por favor, verifique os comentários na plataforma.</p>', esc_html( $post->post_title ) ),
                array(
                    'color'      => $color,
                    'link'       => $edit_link,
    ...
    public function notify_on_author_assignment( int $post_id, \WP_Post $post_after, \WP_Post $post_before ): void {
        ...
        if ( (int) $post_after->post_author !== (int) $post_before->post_author ) {
            ...
            $this->mailer->send(
                ...
                sprintf( '<p>O artigo <strong>"%s"</strong> foi atribuído a você no painel da SULTS.</p>', esc_html( $post_after->post_title ) ),
                array(
                    'color'      => '#00acac',
                    'link'       => get_edit_post_link( $post_after->ID, 'raw' ),
