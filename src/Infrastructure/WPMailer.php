...
class WPMailer implements MailerInterface {
    ...
    public function send( int $user_id, string $subject, string $message, array $options = array() ): bool {
        ...
        <body style="margin: 0; padding: 0; background-color: <?php echo $color_bg_outer; ?>; font-family: sans-serif;">
            <table role="presentation" border="0" cellpadding="0" cellspacing="0" width="100%" style="background-color: <?php echo $color_bg_outer; ?>;">
                <tr>
                    <td align="center" style="padding: 40px 15px;">

                        <table role="presentation" border="0" cellpadding="0" cellspacing="0" width="100%" style="max-width: 600px; background-color: <?php echo $white; ?>; border-radius: 12px; overflow: hidden; box-shadow: 0 4px 10px rgba(0,0,0,0.03);">
                            ...
                            <tr>
                                <td style="padding: 40px;">
                                    <h1 style="color: <?php echo $brand_color; ?>; margin: 0 0 25px 0; font-size: 24px; font-weight: 700;"><?php echo esc_html( $subject ); ?></h1>
                                    
                                    <div style="color: <?php echo $text_color; ?>; font-size: 16px; line-height: 1.6;">
                                        <?php echo $message; ?>
                                    </div>
                            ...
                            <tr>
                                <td style="padding: 20px 40px; background-color: #fafbfc; border-top: 1px solid <?php echo $color_bg_outer; ?>;">
                                    <p style="margin: 0; font-size: 12px; color: <?php echo $muted_text; ?>; text-align: center;">
    ...
    private function get_css_var( string $var_name, string $fallback ): string {
        if ( empty( self::$css_vars ) ) { $this->load_css_variables(); }
    ...
    private function load_css_variables(): void {
        ...
        if ( preg_match_all( '/--([a-zA-Z0-9-]+)\s*:\s*(#[a-fA-F0-9]{3,6})/', $css_content, $matches ) ) {
            foreach ( $matches[1] as $index => $name ) { self::$css_vars[ $name ] = $matches[2][ $index ]; }
        }
    }
}