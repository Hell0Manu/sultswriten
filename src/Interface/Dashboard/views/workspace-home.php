<?php
/**
 * View: Tela inicial do Workspace (Dashboard).
 *
 * @package    Sults\Writen
 * @subpackage Sults\Writen\Interface\Dashboard\Views
 * @since      0.1.0
 *
 * @var \Sults\Writen\Interface\Dashboard\WorkspaceController $this               Instância do controlador.
 * @var \WP_Query                                             $my_posts           Query com os posts do usuário.
 * @var array                                                 $notifications      Lista de notificações.
 * @var int                                                   $unread_count       Contagem de não lidas.
 * @var array                                                 $current_user_roles Roles do usuário atual.
 */
...
<div class="wrap sults-dashboard-wrap">
	<div>
		<h1>DASHBOARD</h1>
		<p style='margin:0'>Bem-vindo ao Sults Writen. Nessa página você encontra os conteúdos atribuídos a você.</p>
	</div>
	...
	<div class="sults-dashboard-grid" style="display: flex; gap: 20px; margin-top: 20px; flex-wrap: wrap;">
						...
						<table class="wp-list-table widefat fixed striped table-view-list posts" style="border: none; box-shadow: none;">
							...
							<tbody>
								...
								while ( $my_posts->have_posts() ) :
									...
									// Lógica de Categoria (Cor e Nome).
									...
									$sultswriten_status_slug = get_post_status();

									// Lógica de Bloqueio: Verifica se o usuário pode editar neste status.
									$sultswriten_is_locked = $this->get_policy()->is_editing_locked( $sultswriten_status_slug, $current_user_roles );

									$sultswriten_edit_url = get_edit_post_link();
									if ( ! $sultswriten_edit_url ) {
										$sultswriten_is_locked = true; // Fallback se o WP negar acesso nativamente.
									}
									?>
									<tr>
										<td>
										...
										<td style="text-align-last: center;">
											<?php
											// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- HTML gerado internamente seguro.
											echo $this->get_policy()->get_status_badge( $sultswriten_status_slug );
											?>
										</td>

										<td style="text-align: right;">
											<?php if ( $sultswriten_is_locked ) : ?>
		...
		<div class="sults-column-side" style="flex: 1; min-width: 280px;">
			<?php
			// Inclui o template parcial de notificações.
			require __DIR__ . '/workspace-notifications.php';
			?>
		</div>
