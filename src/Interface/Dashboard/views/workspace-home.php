<?php
/**
 * View: Tela inicial do Workspace.
 * Acesso disponível: $this (WorkspaceController), $my_posts, $notifications, $unread_count, $current_user_roles.
 */
				...
				<div class="sults-card-body">
					<?php if ( $my_posts->have_posts() ) : ?>
						...
						while ( $my_posts->have_posts() ) :
							...
							$is_locked = $this->policy->is_editing_locked( $status_slug, $current_user_roles );
							...
							?>
								<tr>
									...
									<td>
										<strong>
											<?php if ( ! $is_locked && $edit_url ) : ?>
												<a href="<?php echo esc_url( $edit_url ); ?>" style="color: #1d2327; text-decoration: none;">
													<?php the_title(); ?>
												</a>
											<?php else : ?>
								...
								<?php echo esc_html( get_the_modified_date( 'd/m/Y' ) ); ?>
									...
									<td style="text-align-last: center;">
										<?php echo $this->policy->get_status_badge( $status_slug ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
									</td>
