...
class HierarchyHelper {
     ...
     * @param int    $parent_id ID do pai atual (para recursão).
     ...
     * @param string $parent_prop Nome da propriedade de Pai (ex: 'post_parent' para posts, 'parent' para cats).
     * * @return array
     */
    public static function build_hierarchy( 
        ...
        int $depth = 0,
        string $id_prop = 'ID',
        string $parent_prop = 'post_parent'
    ): array {
        $ordered = array();

        foreach ( $elements as $element ) {
            if ( (int) $element->$parent_prop === $parent_id ) {
